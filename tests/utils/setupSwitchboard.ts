...
export const setupSwitchboard = async (provider, lootboxProgram, payer) => {
  ...
  const { queue, unpermissionedVrfEnabled, authority } = await switchboard.queue.loadData();
  ...
  console.log("\x1b[32m%s\x1b[0m", `\u2714 Switchboard devnet environment loaded successfully\n`);
  ...
  if (!unpermissionedVrfEnabled) {
    if (!payer.publicKey.equals(authority)) {
      throw new Error(`queue requires PERMIT_VRF_REQUESTS and wrong queue authority provided`);
    }
  ...
  const [switchboardStateAccount, switchboardStateBump] = sbv2.ProgramStateAccount.fromSeed(switchboard.program);
